...
% Formatted for CVPR Workshop / NeurIPS Workshop / ECCV Workshop submission
% ============================================================================
...
\ours{} achieves \textbf{30.21\%} average accuracy, outperforming the best CL baseline (NCM, 11.43\%) by \textbf{+18.8~pp}---a $2.65\times$ relative improvement. Remarkably, \ours{} also surpasses the Joint Training upper bound (16.97\%) by \textbf{+13.2~pp}. This result occurs because Joint Training uses a simple MLP trained on all 100 classes simultaneously without progressive learning benefits, whereas \ours{} leverages FM-augmented replay, progressive consolidation, and knowledge distillation that collectively act as implicit data augmentation and curriculum learning.
...
On TinyImageNet, \ours{} achieves \textbf{60.77\%} AA with only \textbf{9.66\%} forgetting---a $3.4\times$ improvement over NCM (17.90\%) and $3.1\times$ over Joint Training (19.40\%). The low forgetting rate (9.66\%) is comparable to iCaRL (7.20\%) and NCM (8.40\%), demonstrating that \ours{} effectively balances stability and plasticity on this more challenging benchmark.
...
Table~\ref{tab:ablation} isolates the contribution of each \ours{} component on Split-CIFAR-100.
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
  ...
  \item \textbf{Herding buffer} (v5c vs.\ v5b) adds +5.80~pp, confirming that exemplar quality significantly impacts replay effectiveness.
...
Figure~\ref{fig:task_evolution} shows per-task accuracy evolution as new tasks are learned. \ours{} maintains substantially higher accuracy on early tasks compared to baselines, with a gradual decay consistent with the 27.68\% forgetting rate. Critically, absolute accuracy remains well above all baselines even for the most forgotten tasks.
